update_kubeconfigs() {
  [ ! -d "$HOME/.kube/config.d" ] && mkdir -p -v "$HOME/.kube/config.d"
  # Will run only if there are new files in the config directory
  local new_files
  new_files=$(find "$HOME/.kube/config.d/" -newer "$HOME/.kube/config" -type f | wc -l)
  if [[ $new_files -ne 0 ]]; then
    ...
    kubeconfig_files="$kubeconfigfile:$(ls "$HOME/.kube/config.d/"* | tr '\n' ':')"
